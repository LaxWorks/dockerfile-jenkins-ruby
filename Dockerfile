...
USER root
RUN passwd -d jenkins && echo 'jenkins ALL=(ALL) ALL' > /etc/sudoers.d/jenkins
RUN ln -sf /usr/share/zoneinfo/Asia/Shanghai /etc/localtime

RUN mkdir /usr/local/rvm
RUN apt-get update \
      && apt-get install -q -y --no-install-recommends curl ca-certificates procps sudo nodejs libpq-dev build-essential \
