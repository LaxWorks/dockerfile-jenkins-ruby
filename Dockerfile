...
RUN sudo apt-get update \
      ...
      && curl -sSL https://rvm.io/pkuczynski.asc | gpg --no-tty --import - \
          ...
          && bash -c 'source $HOME/.rvm/scripts/rvm && rvm requirements && rvm use 2.3.8 --default --install --fuzzy && rvm use 2.6 --default --install --fuzzy && bundle install --gemfile=/tmp/Gemfile && rvm use 2.5 --default --install --fuzzy && bundle install --gemfile=/tmp/Gemfile && rvm cleanup checksums repos logs gemsets links' \
